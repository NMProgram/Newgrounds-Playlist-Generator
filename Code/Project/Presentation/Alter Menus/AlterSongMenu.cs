...
public class AlterSongMenu : AlterMenu
{
    ...
    protected long GetLevelID() => Validate(_prompts[4], x => CheckID(x, _access.IsUniqueLevelID));
    ...
    void Delete()
    {
        long oldID = Validate(_prompts[0].Insert(_prompts[0].Length - 2, " to delete"), x => CheckID(x, _access.IsInDatabase));
        Song song = _access.GetByID(oldID)!;
